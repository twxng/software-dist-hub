...
namespace GigaScramSoft.Services
{
    public class ContentService : IContentUnitService
    {
        ...
        {
            var imagesToRemove = _context.ContentUnitImages.Where(x => x.ContentUnitId == foundContentUnit.Id).ToList();

            _context.ContentUnitImages.RemoveRange(imagesToRemove);

            foundContentUnit.Images.Clear();
            ...
            foreach (var img in updatedImages)
            {
                ...
                await _context.ContentUnitImages.AddAsync(imageModel);
            }
